...
Current camera-side post effects are edited from right `Effect` panel when `対象 = Camera`.
...
- Tone mapping: dropdown at bottom of post effect list

## Hidden / Simplified Items

The following items are intentionally hidden from current UI:
...
- `SSAO` remains disabled in current UI path.

## DoF Notes

- Main DoF uses `DefaultRenderingPipeline.depthOfField`.
- `前後補正` is a signed offset from camera target.
- Positive values move focus nearer to camera.
- Negative values move focus farther behind target.
