...
async function initializeApp(): Promise<void> {
  ...
  try {
    ...
    if (statusText) {
      statusText.textContent = t("common.error.initFailed");
    }
    ...
    if (overlay) {
      ...
      if (title) title.textContent = t("common.error.initFailed");
      if (hint) hint.textContent = t("common.error.details", { message });
    }
  }
}
